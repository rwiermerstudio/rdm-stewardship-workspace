...
Meridian is an invented Dutch institute. The browser lesson follows research work from the person making a change to the colleagues who need to check it. Each project explains why Meridian asks for these steps, what to prepare, who receives it and why it might come back. The page collects a handover note from your choices. References to proposed checks remain unverified, and unanswered questions remain visible. The six cases keep different routes: genetics and rare-site maps stop for outside decisions, while the sky survey continues to a conversation with an archive curator. You can practise reviewer returns, corrections and a returned archive file list. After the route ends, a reflection asks what might make the real process easier; suggested improvements are not implemented capabilities or measured savings. See the [plain-language lesson plan](docs/plans/plain-language-lesson.md) and [verification report](docs/PLAIN-LANGUAGE-LESSON.md).

**No permission is created.** The page loads small authored teaching files, offers selected text checks and creates a local handover archive with SHA-256 digests. It never opens real research files, verifies consent, reserves archive space or deposits anything. Do not type real names, quotes, locations, genomes, consent material or credentials. The package keeps learner proposals, actual synthetic-text findings and unresolved human decisions separate. DataCite and RO-Crate mappings are explicitly partial drafts, not full conformance claims. See [inspectable materials and package verification](demo/MATERIALS.md), [role-play design](docs/ROLEPLAY.md) and [learning specification](docs/LEARNING-REWRITE.md).
