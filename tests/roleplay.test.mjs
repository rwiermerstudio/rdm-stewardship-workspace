...
const start=id=>{let s=fresh(id),c=scenarios[id];s=send(s,'choose',{choice:c.choices.find(x=>x.good).id});s=send(s,'continue');return send(s,'record');};
...
test('generated pointer changes the record but never grants access',()=>{
 const s=start('stellar-survey'),text=draft(s);assert.match(text.record,/sky-raw-v1.*sky-calibrated-v2/s);assert.match(text.record,/EV-STELLAR-SURVEY-001/);assert.match(text.handoff,/capacity/i);assert.equal(s.accessGranted,false);assert.equal(s.publicMetadata,false);
});
test('curator cannot skip review; repair cannot be ceremonial',()=>{
 ...
 assert.throws(()=>send(s,'repair',{plan:'inventory-pending'}));
 s=send(s,'repair',{plan:'inventory-revised'});
