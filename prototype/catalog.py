...
def validate_catalog(data):
    ...
    for policy in data["policies"]:
        ...
        if policy["effect"] == "review" and policy.get("owner") not in persons:
            raise ValueError("review rule requires a known decision owner")
    ...
    for project in projects.values():
        ...
        if project["size_gib"] <= 0 or not project["formats"] or len(project["formats"]) != len(set(project["formats"])):
...
def _guide_input(project):
    return {
        ...
        "community_authority": "required" if project["community_governed"] else "not-applicable",
...
def _active_releases(policies, project, date):
    """Choose one reviewed release per rule ID; reject effective-date ties."""
    active = {}
    ...
    return sorted((p for p in active.values() if _applies(p, project, date)), key=lambda p: p["id"])
...
def evaluate(data, project_id, *, as_of="2026-09-25"):
    ...
    matches = _active_releases(data["policies"], project, date)
    ...
    held = base["outcome"] == "hold" or bool([p for p in matches if p["effect"] == "review"]) or not candidates
    return {
        ...
        "referrals": base["referrals"] + (["preservation specialist: no candidate meets all stated capabilities"] if not candidates else []),
