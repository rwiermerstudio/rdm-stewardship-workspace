...
export function processView(s){
 ...
 const status=r=>r==='curator'&&s.id!=='stellar-survey'?'not in this practice route':r===active?'next':s.reviews[r]?.decision==='return'?'returned':s.reviews[r]?'recorded':r==='researcher'&&s.reference?'drafted':r==='curator'&&s.receipt?.decision==='noted'?'recorded':'waiting';
 ...
 return {roles:rolesView,steps:[{name:'Choose a response',status:s.choice?'drafted':'next'},{name:'Record proposed method',status:s.reference?'drafted':'waiting'},{name:'Independent questions',status:c.reviewers.every(r=>s.reviews[r]&&s.reviews[r].decision!=='return')?'recorded':s.phase==='review'?'next':'waiting'},{name:'Repository question',status:s.id!=='stellar-survey'?'not in this practice route':s.receipt?.decision==='noted'?'recorded':s.phase==='curator'?'next':'waiting'}],open:[...s.issues.filter(x=>x.status==='open').map(x=>`${x.id}: ${x.question}`),c.external,...(c.hold?[c.hold]:[])],fixed:s.issues.filter(x=>x.status==='fixed in draft').map(x=>`${x.id}: ${x.resolution} (${x.evidence}; proposed only)`)};
}
