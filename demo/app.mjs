...
function render(){const cfg=scenarios[state.scenario],v=view(state,role),pending=state.required.filter(r=>state.decisions[r]?.decision!=='accept');
 ...
 for(const key of ['purpose','moment','assets','sample','known','unknown','options','outcome']) text(key,cfg[key]);
 const generated=draft(state);text('change-draft',generated.record);text('handoff-draft',generated.handoff);
 text('status',!state.description?'NOT STARTED':state.receipt?.decision==='accept'?'SIMULATED HANDOFF':state.receipt?.decision==='reject'?'HANDOFF RETURNED':pending.length?'REVIEW IN PROGRESS':cfg.hold && ['coastal-species','variant-study'].includes(state.scenario)?'EXTERNAL HOLD':'AWAITING CURATOR');
